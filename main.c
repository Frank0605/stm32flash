...
int  function(int size, char* arry_function[]) {
	int ret = 1;
	stm32_err_t s_err;
	parser_err_t perr;
	diag = stdout;

	if (parse_options(size, arry_function) != 0)
		goto close;
...
int  parse_options(int size, char *arry_parse[])
{
	int c;
	char *pLen;

	while ((c = getopt(size, arry_parse, "a:b:m:r:w:e:vn:g:jkfcChuos:S:F:i:R")) != -1) {
		switch(c) {
			case 'a':
	...
	for (c = optind; c < size; ++c) {
		if (port_opts.device) {
			fprintf(stderr, "ERROR: Invalid parameter specified\n");
			show_help(arry_parse[0]);
			return 1;
		}
		port_opts.device = arry_parse[c];
	}
